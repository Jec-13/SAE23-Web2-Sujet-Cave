...
function header_page($nom){
    ...
    echo "<h1 class='email_title'>Connecter en tant que : ".$_SESSION['EMAIL']."</h1>"; // affichage de ses autorisation
    echo "<h2 class='page_title'>Sur la page : ".$nom."</h2>"; // affichage du nom de la page
    echo "<p class='role_title'>Votre profile est ".$role."</p>";
    ?>
    <p><a class="btn-deconnexion" href='INCLUDE/deco.php'>se déconecter</a></p> <!--bouton de déconnexion et qui redirige vers deco.php-->
    </div>
    <?php
}
...
function menu_page(){
    ...
    <?php 
    if ($_SESSION['ADMIN']){ // si l'utilisateur est admin alors il a le droit d'accèder au page suivante :
