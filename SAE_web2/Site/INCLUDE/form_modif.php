<?php
function from_modif($name, $path, $type){
    ?>
    <div id="tableau">
    <nav class="rect">
    ...
    <form id='form1' method='POST' onsubmit='return false'>
        ...
        <div class='champ'>
        <input class='btn' type='button' value='Changer' onclick='modifier(event, "Nb_bouteilles", <?php echo $type; ?>)' />
    ...
    if (isset($_POST['Nb_bouteilles'])){
        if ($_POST['Nb_bouteilles'] == $name["NB_BOUTEILLES"]){
            echo "<p class='erch' style='display:block'>Le nombre doit être différent de celui actuel</p>";
        } else {
