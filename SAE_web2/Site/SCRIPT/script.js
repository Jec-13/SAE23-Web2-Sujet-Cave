...
function isEmailValid(email) {
    ...
    $retour = true;
    const regex = /^[\w.-]+@[\w.-]+\.[a-zA-Z]{2,}$/; // adresse email format caractères@caractères.lettres(2 ou +)
    if (!regex.test(email)) {
...
function ChangeDisplay(el) {
    // afficher le message d'erreur avec l'id el
...
async function modifier(form, nego, cru, coord, nb_prec) {
    ...
    // coord = coordonnée du bon élément du captcha
    // nb_prec = nombre de bouteilles avant modification

    // récupération du nombre de bouteilles et de la case cochée
    let Nb_bouteilles = form.elements["Nb_bouteilles"].value;
    let scase = form.elements["case"].value;

    if (Nb_bouteilles==""){ // vérification que le champ bouteilles n'est pas vide
        ChangeDisplay('remp');
        document.getElementById('succes').style.display = 'none'; // pas de succes donc si avant il y en avait un on l'enlève
    } else if (scase==""){ // vérification qu'une case a bien été cliquée
        ChangeDisplay('ercap');
        document.getElementById('succes').style.display = 'none'; // pas de succes donc si avant il y en avait un on l'enlève
    } else if (Nb_bouteilles == nb_prec || Nb_bouteilles<0){ // vérification que le nombre de bouteilles à modifier est bien différent de l'ancien et qu'il soit positif
        ChangeDisplay('erch');
        document.getElementById('succes').style.display = 'none'; // pas de succes donc si avant il y en avait un on l'enlève
    }
    else{
        try {
            const reponse = await fetch('INCLUDE/fetch_modif.php', { // construction de la requête fetch
                ...
                body: "Nb_bouteilles="+Nb_bouteilles+"&NAME="+nego+"&CRU="+cru+"&scase="+scase+"&coord="+coord // éléments nécessaires pour la modification
            });
            ...
            // on enlève les erreurs si on réussit
